...
public class MyCustomScreen extends Screen {
    ...
    protected void init() {
        ...
        // 繪製關閉選單的按鈕 
        this.addDrawableChild(ButtonWidget.builder(Text.literal("關閉選單"), button -> this.close())
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int titleColor = ColorUtils.fixColor(0xFFAA00);
        context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("指令自定義面板"), this.width / 2, 20, titleColor);
        int titleColor2 = ColorUtils.fixColor(0xFFFFFF);
        context.drawTextWithShadow(this.textRenderer, Text.literal("請輸入指令 (不需 /):"), this.width / 2 - 100, 40, titleColor2);
    }
    ...
    /**
     * 確保顏色代碼包含不透明的 Alpha 通道 (0xFF)
     * 符合 1.21.11 DrawContext 渲染規範
     */
    public static int fixColor(int color) {
        // 如果顏色小於等於 0xFFFFFF (只有 RGB)，則補上最前面的 FF (Alpha)
    ...
    // 常用 1.21.11 標准色
    public static final int MC_GOLD = 0xFFFFAA00;
    public static final int MC_WHITE = 0xFFFFFFFF;
    public static final int MC_RED = 0xFFFF5555;
    }
